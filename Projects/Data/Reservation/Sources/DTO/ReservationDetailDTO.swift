...
extension ReservationDetailDTO {
    var toDomain: ReservationDetail {
        .init(
            ...
            reservationDatetime: self.reservationDatetime.toDate(with: .iso8601) ?? .now,
            ...
            createdAt: self.createdAt.toDate(with: .iso8601) ?? .now,
            updatedAt: self.updatedAt.toDate(with: .iso8601) ?? .now
        )
    }
}
